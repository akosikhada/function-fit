      ...
      try {
        ...
        if (insertError) {
          ...
          if (insertError.code === "23505") {
            // Fetch the stats that were created by a concurrent process
            const { data: concurrentStats, error: fetchError } = await supabase
              .from("user_stats")
              .select("*")
              .eq("user_id", userId)
              .eq("date", today)
            ...
            if (concurrentStats) {
              // Update with the new steps value
              const { error: updateError } = await supabase
                .from("user_stats")
                .update({
                  steps: steps,
        ...
        if (catchError.code !== "23505") {
          console.error("Error tracking steps:", catchError);
          throw catchError;
        }
      }
    }
...
export const trackCalories = async (userId: string, calories: number) => {
  try {
    ...
    if (isMockUser) {
      ...
      if (!existingUser) {
        console.log(
          "Creating mock user profile for tracking calories:",
          userId
        );
    ...
    if (existingStats) {
      ...
      console.log(
        `Updated calories for user ${userId}: ${updatedCalories} total`
      );
      ...
      // Create new stats for today
      try {
        const { error: insertError } = await client.from("user_stats").insert({
          user_id: userId,
          date: today,
          steps: 0,
          calories: calories,
        ...
        if (insertError) {
          ...
          if (insertError.code === "23505") {
            // Try to fetch the stats again and update them
            ...
            if (concurrentStats) {
              // Update the stats with the new calories
              const { error: updateError } = await client
                .from("user_stats")
                .update({
              ...
              console.log(
                `Updated calories for concurrent stats: ${
                  concurrentStats.calories + calories
                } total`
              );
              return {
                success: true,
                updatedValue: concurrentStats.calories + calories,
              };
            }
          } else {
            console.error(
              "Error creating user stats with calories:",
              insertError
            );
            throw insertError;
          }
        ...
        console.log(
          `Created new stats entry for user ${userId} with ${calories} calories`
        );
        ...
        if (catchError.code !== "23505") {
          console.error("Error in trackCalories:", catchError);
          throw catchError;
        }

        // For duplicate key errors not handled above
        ...
        return {
          success: true,
          updatedValue: stats ? stats.calories : calories,
        };
      }
    }
  } catch (error) {
...
export const scheduleWorkout = async (
  ...
  try {
    ...
    if (isMockUser) {
      ...
      if (!existingUser) {
        console.log(
          "Creating mock user profile for workout scheduling:",
          userId
        );
        const { error: createUserError } = await client.from("users").insert({
          id: userId,
    ...
    console.log(
      `Scheduling workout ${workoutId} for user ${userId} on ${date}`
    );
