        ...
        if (existingStats) {
          ...
          try {
            ...
            if (error && error.code === "23505") {
              ...
              const { data: concurrentStats, error: fetchError } =
                await supabase
                  .from("user_stats")
                  .select("*")
                  .eq("user_id", user.id)
              ...
              if (!fetchError && concurrentStats) {
                // Update with the incremented workouts_completed
                await supabase
                  .from("user_stats")
                  .update({
                    workouts_completed: concurrentStats.workouts_completed + 1,
                    updated_at: new Date().toISOString(),
                  })
            ...
            // Only log if it's not a duplicate key error we couldn't handle
            if (error.code !== "23505") {
