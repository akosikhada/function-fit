...
import { ChevronLeft, Clock, XCircle, Dumbbell, ArrowRight, Heart, Flame, CheckCircle2 } from "lucide-react-native";
...
export default function WorkoutPlayerScreen() {
      ...
      if (isResting) {
        ...
        try {
          if (currentExercise?.rest) {
            ...
            const restText = String(currentExercise.rest).replace(/rest|s$/gi, "").trim();
  ...
  const calculateProgress = () => {
    ...
    return Math.min(Math.floor((completedExercises / totalExercises) * 100), 100);
  };
    ...
    const fetchWorkout = async () => {
      try {
        ...
        const formattedWorkout: WorkoutData = {
          ...
          imageUrl: workoutData?.image_url || "https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=500&q=80",
    ...
    try {
      ...
      if (user && workout) {
        try {
          ...
          const calorieResult = await trackCaloriesAPI(user.id, estimatedCalories);
          const updatedCalories = calorieResult?.updatedValue || estimatedCalories;
          
          // Calculate progress towards daily goal of 600 calories
          const calorieProgress = Math.min(Math.round((updatedCalories / 600) * 100), 100);
          ...
          // Show success toast
          setToastMessage(`${estimatedCalories} calories added to your dashboard!`);
    ...
    if (timerActive) {
      timer = setInterval(() => {
        ...
        setCaloriesBurned(prev => {
          ...
          if (!isResting && workout?.exercises && workout.exercises[currentExerciseIndex]) {
            const currentExercise = workout.exercises[currentExerciseIndex];
            ...
            } else if (currentExercise.reps && currentExercise.reps.includes('15')) {
              intensityMultiplier = 1.2; // Medium-high intensity
            } else if (currentExercise.reps && currentExercise.reps.includes('10')) {
  ...
  const nextExercise = workout && currentExerciseIndex < workout.exercises.length - 1 
  ...
  const getTimerSize = () => {
    if (isLargeScreen) return SCREEN_WIDTH * 0.6 > 300 ? 300 : SCREEN_WIDTH * 0.6;
  ...
  const trackCaloriesBurned = async (userId: string, calories: number, totalToday: number) => {
    try {
      console.log(`Logged ${calories} calories for user ${userId}. Total today: ${totalToday}`);
  ...
  const getUser = async (): Promise<any | null> => {
    try {
      // In a real app, this would retrieve user from authentication
      const userData = await AsyncStorage.getItem('current_user');
      
      if (!userData) {
        // For development, use a real user ID from the database instead of a mock one
        const mockUser = {
          id: '87aaa165-b8d2-4892-9c9b-86a845b66ac3', // Use a real user ID from the database
          name: 'Test User',
  ...
  const checkForDayChange = async (userId: string): Promise<boolean> => {
    try {
      ...
      if (lastTrackedDate !== today) {
        console.log(`Workout player: Day changed. Last: ${lastTrackedDate}, Today: ${today}`);
  ...
  const trackCalories = async (userId: string, caloriesBurned: number): Promise<{ success: boolean, totalToday: number }> => {
    try {
      ...
      try {
        ...
        await trackCaloriesBurned(userId, caloriesBurned, result?.updatedValue || newTotal);
        ...
        <View style={{
          ...
          <View style={{
            ...
            <Text style={{ 
              ...
              +{updatedCalories ? Math.round(updatedCalories) : Math.round(caloriesBurned)} calories
            </Text>
          </View>
          ...
          <View style={{
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 }}>
              <Text style={{ color: colors.secondaryText, fontSize: 12 }}>Daily Goal</Text>
              <Text style={{ color: colors.secondaryText, fontSize: 12 }}>600 calories</Text>
            </View>
            <View style={{
              height: 8,
              backgroundColor: isDarkMode ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)',
          ...
          <Animated.View style={{ 
            ...
            <View style={{
              ...
              <Text style={{ 
                ...
                {isResting ? "REST PERIOD" : `EXERCISE ${currentExerciseIndex + 1}/${workout?.exercises.length || 0}`}
              </Text>
            </View>
            ...
            {!isResting && currentActivity && (
              <View style={{
                ...
                <Dumbbell size={getFontSize(12, 14, 16)} color={colors.secondaryText} style={{ marginRight: 8 }} />
                <Text style={{ 
                  color: colors.secondaryText, 
                  textAlign: "center",
                  fontSize: getFontSize(12, 14, 16)
          ...
          <View style={{ 
            ...
            {nextExercise ? (
                ...
                <View style={{ 
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap' }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', marginRight: 10, marginBottom: isSmallScreen ? 4 : 0 }}>
                        <Clock size={getFontSize(12, 14, 16)} color={colors.secondaryText} style={{ marginRight: 4 }} />
                        <Text style={{ 
                          color: colors.secondaryText, 
                          fontSize: getFontSize(12, 14, 16)
                        }}>
                  ...
                  {!isSmallScreen && (
                    <ArrowRight size={getFontSize(16, 20, 24)} color={getAccentColor()} />
                  )}
                </View>
              </View>
            ) : (
              <View style={{ 
                flexDirection: 'row', 
                alignItems: 'center',
                backgroundColor: isDarkMode ? 'rgba(255,255,255,0.08)' : 'rgba(99, 102, 241, 0.1)',
                width: '100%',
                borderRadius: 12,
                padding: getSpacing(12, 16, 20),
                marginBottom: getSpacing(8, 12, 16)
              }}>
                <Heart size={getFontSize(20, 24, 28)} color={getAccentColor()} style={{ marginRight: getSpacing(8, 12, 16) }} />
                <Text style={{ 
                  color: colors.text, 
                  fontWeight: "600",
            ...
            <View style={{ marginTop: getSpacing(8, 12, 16) }}>
                ...
                <View style={{ paddingBottom: 16 }}>
                  {workout.exercises.slice(currentExerciseIndex + 1).map((exercise, index) => (
                    <View key={exercise.id} style={{
                      ...
                      borderBottomWidth: index < workout.exercises.slice(currentExerciseIndex + 1).length - 1 ? 1 : 0,
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                          ...
                          <Text style={{ color: colors.secondaryText, marginHorizontal: 4 }}>•</Text>
                          <Text style={{
                            fontSize: getFontSize(11, 12, 14),
                            color: colors.secondaryText,
                          }}>
              ...
              <View style={{
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                  <Text style={{ color: colors.secondaryText, fontSize: getFontSize(12, 13, 14) }}>
                    Total Exercises
                  </Text>
                  <Text style={{ color: colors.text, fontSize: getFontSize(12, 13, 14), fontWeight: '600' }}>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                  ...
                  <Text style={{ color: colors.text, fontSize: getFontSize(12, 13, 14), fontWeight: '600' }}>
                    {Math.min(currentExerciseIndex + (isResting ? 1 : 0), workout?.exercises?.length || 0)}
                  </Text>
                </View>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                  ...
                  <Text style={{ color: colors.text, fontSize: getFontSize(12, 13, 14), fontWeight: '600' }}>
                    {formatTime(workout?.exercises?.slice(currentExerciseIndex + 1).reduce((total, ex) => {
