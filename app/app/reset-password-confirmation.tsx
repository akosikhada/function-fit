...
export default function ResetPasswordConfirmationScreen() {
  ...
  // Get access token and refresh token from URL params if they exist
  const accessToken = params.access_token as string;
  const refreshToken = params.refresh_token as string;
  const type = params.type as string;
  const email = params.email as string;
  const devReset = params.dev_reset as string;
  
  // Initialize session if tokens are available or if in dev testing mode
  useEffect(() => {
    const setSession = async () => {
      // Handle standard password reset from email link
      if (accessToken && type === "recovery") {
        try {
          ...
          if (error) {
            showErrorToast("Invalid or expired recovery link. Please request a new password reset.");
          }
        } catch (error) {
          showErrorToast("Error setting up recovery session.");
        }
      }
      // Handle development testing flow
      else if (devReset === "true" && email) {
        // In development mode, we'll allow direct password reset for testing
        // This is a shortcut for development only and should be removed in production
        console.log("Development password reset mode for email:", email);
      }
    };
    
    setSession();
  }, [accessToken, refreshToken, type, devReset, email]);
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      ...
      if (devReset === "true" && email) {
        // For development testing: use admin reset approach
        // This would normally be done through the email link flow
        try {
          // First, reset the user's password with a temporary one
          ...
          throw new Error("Development password reset failed. Make sure you're using a valid email.");
        }
      } else {
        // Normal flow - user clicked the email link
        const { error } = await supabase.auth.updateUser({
          password: newPassword
        });
